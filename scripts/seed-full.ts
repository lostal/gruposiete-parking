...
const log = {
  ...
  warning: (msg: string) =>
    console.warn(
      `${colors.yellow}${colors.bright}ADVERTENCIA:${colors.reset} ${msg}`
    ),
  success: (msg: string) =>
    console.log(`${colors.green}${colors.bright}✓${colors.reset} ${msg}`),
  info: (msg: string) => console.log(`${colors.blue}ℹ${colors.reset} ${msg}`),
  step: (msg: string) =>
    console.log(`${colors.cyan}${colors.bright}→${colors.reset} ${msg}`),
};
...
function verificarEntornoSeguro(): void {
  ...
  if (process.env.NODE_ENV === "production") {
    log.error(
      "Este script NO puede ejecutarse en producción (NODE_ENV=production)"
    );
    process.exit(1);
  }
  ...
  const contieneProduccion = produccionKeywords.some((keyword) =>
    mongoUri.includes(keyword)
  );

  if (contieneProduccion) {
    log.error("La URI de MongoDB parece ser de PRODUCCIÓN");
    log.error(
      `Palabras detectadas: ${produccionKeywords
        .filter((k) => mongoUri.includes(k))
        .join(", ")}`
    );
...
function generarFechas(
  ...
  for (let i = 0; i < cantidad; i++) {
    const diasOffset =
      Math.floor(Math.random() * (diasAtras + diasAdelante)) - diasAtras;
...
async function crearPlazasParking() {
  ...
  const usuariosDireccion = await User.find({ role: UserRole.DIRECCION }).sort({
    createdAt: 1,
  });
  ...
  for (let i = 0; i < plazasParking.length; i++) {
    ...
    const usuarioAsignado =
      i < usuariosDireccion.length ? usuariosDireccion[i] : null;
    ...
    if (usuarioAsignado) {
      ...
      log.info(
        `  • Plaza ${plaza.number} (${plaza.location}) → ${usuarioAsignado.name}`
      );
    } else {
...
async function crearReservas() {
  ...
  for (let i = 0; i < reservasPorCrear; i++) {
    const usuario =
      usuariosGenerales[Math.floor(Math.random() * usuariosGenerales.length)];
  ...
  log.success(
    `${reservas.length} reservas creadas (${activas} activas, ${canceladas} canceladas)`
  );
  return reservas;
}
...
async function crearDisponibilidades() {
  ...
  const usuariosDireccion = await User.find({
    role: UserRole.DIRECCION,
  ...
  for (const usuario of usuariosDireccion) {
    ...
    log.info(
      `  • ${usuario.name}: ${diasAMarcar} días marcados como disponibles`
    );
  }
...
async function mostrarResumen() {
  ...
  const direccionCount = await User.countDocuments({
    role: UserRole.DIRECCION,
  });
  ...
  const plazasExterior = await ParkingSpot.countDocuments({
    location: ParkingLocation.EXTERIOR,
  });
  ...
  console.log(
    `    • Administradores: ${colors.yellow}${adminCount}${colors.reset}`
  );
  console.log(
    `    • Dirección: ${colors.yellow}${direccionCount}${colors.reset}`
  );
  console.log(
    `    • Generales: ${colors.yellow}${generalCount}${colors.reset}`
  );
  console.log("");
  ...
  console.log(
    `    • Subterráneo: ${colors.yellow}${plazasSubterraneo}${colors.reset}`
  );
  console.log(
    `    • Exterior: ${colors.yellow}${plazasExterior}${colors.reset}`
  );
  console.log("");
  ...
  console.log(
    `    • Activas: ${colors.green}${reservasActivas}${colors.reset}`
  );
  console.log(
    `    • Canceladas: ${colors.red}${reservasCanceladas}${colors.reset}`
  );
  console.log("");

  console.log(`  ${colors.bright}Disponibilidades:${colors.reset}`);
  console.log(
    `    • Total: ${colors.cyan}${totalDisponibilidades}${colors.reset}`
  );
  console.log("");

  console.log(`  ${colors.bright}Credenciales de acceso:${colors.reset}`);
  console.log(
    `    • Email: ${colors.cyan}[cualquier email creado]@gruposiete.es${colors.reset}`
  );
  console.log(
    `    • Password: ${colors.cyan}${DEFAULT_PASSWORD}${colors.reset}`
  );
  console.log("");
  console.log(`    Ejemplos:`);
  console.log(
    `      Admin:      ${colors.yellow}admin@gruposiete.es${colors.reset}`
  );
  console.log(
    `      Dirección:  ${colors.yellow}adrian@gruposiete.es${colors.reset}`
  );
  console.log(
    `      General:    ${colors.yellow}maria@gruposiete.es${colors.reset}`
  );
  console.log("");
}
...
async function main() {
  ...
  console.log(
    `${colors.bright}  SEED COMPLETO - GRUPO SIETE PARKING${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.blue}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`
  );
